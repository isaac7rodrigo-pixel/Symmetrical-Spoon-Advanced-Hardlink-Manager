...
pub fn analyze_path(path: impl AsRef<Path>) -> io::Result<ItemAnalysis> {
    ...
    let link_state = link_state_from_path(path, &metadata)?;
...
fn analyze_folder_contents(root: &Path) -> io::Result<FolderSummary> {
    ...
    while let Some(dir) = queue.pop_front() {
        for entry in fs::read_dir(dir)? {
            ...
            scanned_entries += 1;
            if link_state_from_path(&path, &metadata)? == LinkState::Hardlinked {
...
#[cfg(unix)]
fn link_state_from_path(_path: &Path, metadata: &fs::Metadata) -> io::Result<LinkState> {
    use std::os::unix::fs::MetadataExt;

    Ok(if metadata.nlink() > 1 {
...
#[cfg(windows)]
fn link_state_from_path(path: &Path, _metadata: &fs::Metadata) -> io::Result<LinkState> {
    use std::ffi::c_void;
